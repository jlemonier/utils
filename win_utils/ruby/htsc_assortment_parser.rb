...
startdir = "D:/GT/Projects/HTSC_Fix_Aug_2013/test_3"
Dir["#{startdir}/*"].each do |f|
	text = File.read(f)
	mssg_doc_id = f.split("__", 2)[0].split('/')[-1]	# assuming filename such as: somepath/12345__somefile.txt to get 12345 as mssg_doc_id
	header = [mssg_doc_id]
	results = hp.get_htsc_fix_results text, header
	
	results.each {|r| hp.print_csv r }
end

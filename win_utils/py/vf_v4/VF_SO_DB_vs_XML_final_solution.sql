-- select top 2 * from #temp_inv_gross_amt 

-- 2643 invoices will be correct removing assortments
select vf2.invoicenum, sum(vf2.gross_amt_sku_asst) gross_db_filtered, amt.gross_amt
	-- into #inv_correct
	from #vf2 vf2
	inner join #temp_inv_gross_amt amt on amt.invoicenum = vf2.invoicenum
	group by vf2.invoicenum, amt.gross_amt
	having abs(amt.gross_amt - sum(vf2.gross_amt_sku_asst)) > .001
	order by invoicenum

select * from #inv_correct

select * 
	from #inv_correct i
	inner join #vf vf	on  vf.invoicenum = i.invoicenum
						and	vf.quantity_so_xml is null




-- select * from #vf2 vf2 inner join #temp_inv_gross_amt amt on amt.invoicenum = vf2.invoicenum

-- v5 csv loaded with PoLineNumber correctly - 

if 'create #vf' = 'create #vf' begin
select 'created #vf'
drop table #vf
